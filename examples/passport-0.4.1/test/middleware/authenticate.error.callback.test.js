...
it('-147-should pass error to callback', function() {
      expect(error).to.be.an.instanceOf(Error);
      expect(error.message).to.equal('something is wrong');
    });
    
it('-148-should pass user as undefined to callback', function() {
      expect(request.user).to.be.undefined;
    });
    
it('-149-should not set user on request', function() {
      expect(request.user).to.be.undefined;
    });
  });
...
it('-150-should pass error to callback', function() {
      expect(error).to.be.an.instanceOf(Error);
      expect(error.message).to.equal('something is wrong');
    });
    
it('-151-should pass user as undefined to callback', function() {
      expect(request.user).to.be.undefined;
    });
    
it('-152-should not set user on request', function() {
      expect(request.user).to.be.undefined;
    });
  });
