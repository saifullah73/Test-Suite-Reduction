...
it('-258-should not set user', function() {
      expect(request.user).to.be.undefined;
    });
  
it('-259-should respond', function() {
      expect(response.statusCode).to.equal(401);
      expect(response.body).to.equal('Unauthorized');
    });
    
it('-260-should set authenticate header on response', function() {
...
it('-261-should not set user', function() {
      expect(request.user).to.be.undefined;
    });
  
it('-262-should respond', function() {
...
it('-263-should not set user', function() {
      expect(request.user).to.be.undefined;
    });
    
it('-264-should flash message', function() {
      expect(request.message.type).to.equal('error');
      expect(request.message.msg).to.equal('A message');
    });
  
it('-265-should redirect', function() {
...
it('-266-should not error', function() {
      expect(error).to.be.null;
    });
    
it('-267-should pass false to callback', function() {
      expect(user).to.equal(false);
    });
    
it('-268-should pass challenges to callback', function() {
...
it('-269-should pass statuses to callback', function() {
...
it('-270-should not set user on request', function() {
      expect(request.user).to.be.undefined;
    });
  });
...
it('-271-should not error', function() {
      expect(error).to.be.null;
    });
    
it('-272-should pass false to callback', function() {
      expect(user).to.equal(false);
    });
    
it('-273-should pass challenges to callback', function() {
      expect(challenge).to.be.an('array');
      expect(challenge).to.have.length(3);
      expect(challenge[0]).to.equal('BASIC challenge');
      expect(challenge[1]).to.be.undefined;
...
it('-274-should pass statuses to callback', function() {
      expect(status).to.be.an('array');
      expect(status).to.have.length(3);
...
it('-275-should not set user on request', function() {
      expect(request.user).to.be.undefined;
    });
  });
...
it('-276-should not error', function() {
      expect(error).to.be.null;
    });
    
it('-277-should pass false to callback', function() {
      expect(user).to.equal(false);
    });
    
it('-278-should pass challenges to callback', function() {
      expect(challenge).to.be.an('array');
      expect(challenge).to.have.length(1);
      expect(challenge[0]).to.equal('BASIC challenge');
    });
    
it('-279-should pass statuses to callback', function() {
      expect(status).to.be.an('array');
      expect(status).to.have.length(1);
      expect(status[0]).to.be.undefined;
    });
    
it('-280-should not set user on request', function() {
      expect(request.user).to.be.undefined;
    });
  });
