...
it('-391-should not error', function() {
      expect(error).to.be.undefined;
    });
    
it('-392-should set user', function() {
      expect(request.user).to.be.an('object');
      expect(request.user.id).to.equal('1');
      expect(request.user.username).to.equal('jaredhanson');
    });
    
it('-393-should set authInfo', function() {
...
it('-394-should not error', function() {
      expect(error).to.be.undefined;
    });
    
it('-395-should set user', function() {
      expect(request.user).to.be.an('object');
      expect(request.user.id).to.equal('1');
      expect(request.user.username).to.equal('jaredhanson');
    });
    
it('-396-should set authInfo', function() {
      expect(request.authInfo).to.be.an('object');
...
it('-397-should error', function() {
      expect(error).to.be.an.instanceOf(Error);
      expect(error.message).to.equal('something went wrong');
    });
    
it('-398-should set user', function() {
      expect(request.user).to.be.an('object');
      expect(request.user.id).to.equal('1');
      expect(request.user.username).to.equal('jaredhanson');
    });
    
it('-399-should not set authInfo', function() {
...
it('-400-should not error', function() {
      expect(error).to.be.undefined;
    });
    
it('-401-should set user', function() {
      expect(request.user).to.be.an('object');
      expect(request.user.id).to.equal('1');
      expect(request.user.username).to.equal('jaredhanson');
    });
    
it('-402-should not set authInfo', function() {
      expect(request.authInfo).to.be.undefined;
    });
  });
