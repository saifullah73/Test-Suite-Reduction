...
it('-567-should get the version', function () {
    expect(/\d+\.\d+\.\d+/.test(cheerio.version)).to.be.ok();
  });

it('-568-cheerio(null) should return be empty', function () {
    expect(cheerio(null)).to.be.empty();
  });

it('-569-cheerio(undefined) should be empty', function () {
    expect(cheerio(undefined)).to.be.empty();
  });

it('-570-cheerio(null) should be empty', function () {
    expect(cheerio('')).to.be.empty();
  });

it('-571-cheerio(selector) with no context or root should be empty', function () {
    expect(cheerio('.h2')).to.be.empty();
    expect(cheerio('#fruits')).to.be.empty();
  });

it('-572-cheerio(node) : should override previously-loaded nodes', function () {
...
it('-573-should be able to create html without a root or context', function () {
    var $h2 = cheerio('<h2>');
...
it('-574-should be able to create complicated html', function () {
...
it('-575-should be able to select .apple with only a context', function () {
    var $apple = cheerio('.apple', fruits);
    testAppleSelect($apple);
  });

it('-576-should be able to select .apple with a node as context', function () {
    var $apple = cheerio('.apple', cheerio(fruits)[0]);
    testAppleSelect($apple);
  });

it('-577-should be able to select .apple with only a root', function () {
    var $apple = cheerio('.apple', null, fruits);
    testAppleSelect($apple);
  });

it('-578-should be able to select an id', function () {
    var $fruits = cheerio('#fruits', null, fruits);
    expect($fruits).to.have.length(1);
    expect($fruits[0].attribs.id).to.equal('fruits');
  });

it('-579-should be able to select a tag', function () {
    var $ul = cheerio('ul', fruits);
    expect($ul).to.have.length(1);
    expect($ul[0].tagName).to.equal('ul');
  });

it('-580-should accept a node reference as a context', function () {
    var $elems = cheerio('<div><span></span></div>');
    expect(cheerio('span', $elems[0])).to.have.length(1);
  });

it('-581-should accept an array of node references as a context', function () {
    var $elems = cheerio('<div><span></span></div>');
    expect(cheerio('span', $elems.toArray())).to.have.length(1);
  });

it('-582-should select only elements inside given context (Issue #193)', function () {
...
it('-583-should be able to select multiple tags', function () {
...
it('-584-should be able to do: cheerio("#fruits .apple")', function () {
    var $apple = cheerio('#fruits .apple', fruits);
    testAppleSelect($apple);
  });

it('-585-should be able to do: cheerio("li.apple")', function () {
    var $apple = cheerio('li.apple', fruits);
    testAppleSelect($apple);
  });

it('-586-should be able to select by attributes', function () {
    var $apple = cheerio('li[class=apple]', fruits);
    testAppleSelect($apple);
  });

it('-587-should be able to select multiple classes: cheerio(".btn.primary")', function () {
    var $a = cheerio('.btn.primary', multiclass);
    expect($a).to.have.length(1);
    expect($a[0].childNodes[0].data).to.equal('Save');
  });

it('-588-should not create a top-level node', function () {
    var $elem = cheerio('* div', '<div>');
    expect($elem).to.have.length(0);
  });

it('-589-should be able to select multiple elements: cheerio(".apple, #fruits")', function () {
...
it('-590-should be able to select immediate children: cheerio("#fruits > .pear")', function () {
...
it('-591-should be able to select immediate children: cheerio(".apple + .pear")', function () {
...
it('-592-should be able to select immediate children: cheerio(".apple ~ .pear")', function () {
    var $elem = cheerio('.apple ~ li', fruits);
...
it('-593-should handle wildcards on attributes: cheerio("li[class*=r]")', function () {
...
it('-594-should handle beginning of attr selectors: cheerio("li[class^=o]")', function () {
    var $elem = cheerio('li[class^=o]', fruits);
    expect($elem).to.have.length(1);
    expect($elem.eq(0).attr('class')).to.equal('orange');
  });

it('-595-should handle beginning of attr selectors: cheerio("li[class$=e]")', function () {
...
it('-596-should gracefully degrade on complex, unmatched queries', function () {
    var $elem = cheerio('Eastern States Cup #8-fin&nbsp;<br>Downhill&nbsp;');
    expect($elem).to.have.length(0); // []
  });

it('-597-(extended Array) should not interfere with prototype methods (issue #119)', function () {
    var extended = [];
...
it('-598-should set html(number) as a string', function () {
    var $elem = cheerio('<div>');
    $elem.html(123);
    expect(typeof $elem.text()).to.equal('string');
  });

it('-599-should set text(number) as a string', function () {
    var $elem = cheerio('<div>');
    $elem.text(123);
    expect(typeof $elem.text()).to.equal('string');
  });
...
it('-600-should generate selections as proper instances', function () {
      var $ = cheerio.load(fruits);

      expect($('.apple')).to.be.a($);
    });

it('-601-should be able to filter down using the context', function () {
      var $ = cheerio.load(fruits),
          apple = $('.apple', 'ul'),
          lis = $('li', 'ul');
...
it('-602-should allow loading a pre-parsed DOM', function () {
...
it('-603-should render xml in html() when options.xml = true', function () {
...
it('-604-should render xml in html() when options.xml = true passed to html()', function () {
      var str = '<MixedCaseTag UPPERCASEATTRIBUTE=""></MixedCaseTag>',
...
it('-605-should respect options on the element level', function () {
      var str =
...
it('-606-should return a fully-qualified Function', function () {
...
it('-607-should honor extensions defined on `prototype` property', function () {
        var $ = cheerio.load('<div>');
        var $div;
...
it('-608-should honor extensions defined on `fn` property', function () {
        var $ = cheerio.load('<div>');
        var $div;
...
it('-609-should isolate extensions between loaded functions', function () {
