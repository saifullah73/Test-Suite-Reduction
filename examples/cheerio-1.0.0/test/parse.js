...
it('-610-should parse basic empty tags: ' + basic, function () {
...
it('-611-should handle sibling tags: ' + siblings, function () {
...
it('-612-should handle single tags: ' + single, function () {
...
it('-613-should handle malformatted single tags: ' + singleWrong, function () {
...
it('-614-should handle tags with children: ' + children, function () {
...
it('-615-should handle tags with children: ' + li, function () {
...
it('-616-should handle tags with attributes: ' + attributes, function () {
...
it('-617-should handle value-less attributes: ' + noValueAttribute, function () {
...
it('-622-should handle style tags: ' + style, function () {
...
it('-623-should handle directives: ' + directive, function () {
...
it('-624-should add root to: ' + basic, function () {
...
it('-625-should add root to: ' + siblings, function () {
...
it('-626-should add root to: ' + comment, function () {
...
it('-627-should add root to: ' + text, function () {
...
it('-628-should add root to: ' + scriptEmpty, function () {
...
it('-629-should add root to: ' + styleEmpty, function () {
...
it('-630-should add root to: ' + directive, function () {
...
it('-631-should expose the DOM level 1 API', function () {
      var root = parse(
...
it('-632-Should parse less than or equal sign sign', function () {
...
it('-633-Should ignore unclosed CDATA', function () {
      var root = parse(
...
it('-634-Should add <head> to documents', function () {
...
it('-635-Should implicitly create <tr> around <td>', function () {
...
it('-636-Should parse custom tag <line>', function () {
...
it('-637-Should properly parse misnested table tags', function () {
      var root = parse(
...
it('-638-Should correctly parse data url attributes', function () {
      var html =
...
it('-639-Should treat <xmp> tag content as text', function () {
...
it('-640-Should correctly parse malformed numbered entities', function () {
...
it('-641-Should correctly parse mismatched headings', function () {
...
it('-642-Should correctly parse tricky <pre> content', function () {
      var root = parse(
...
it('-643-should pass the options for including the location info to parse5', function () {
      var root = parse(
        '<p>Hello</p>',
