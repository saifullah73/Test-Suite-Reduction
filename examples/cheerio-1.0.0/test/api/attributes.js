...
it('-0-() : should get all the attributes', function () {
      var attrs = $('ul').attr();
      expect(attrs.id).to.equal('fruits');
    });

it('-1-(invalid key) : invalid attr should get undefined', function () {
      var attr = $('.apple').attr('lol');
      expect(attr).to.be(undefined);
    });

it('-2-(valid key) : valid attr should get value', function () {
      var cls = $('.apple').attr('class');
      expect(cls).to.equal('apple');
    });

it('-3-(valid key) : valid attr should get name when boolean', function () {
      var attr = $('<input name=email autofocus>').attr('autofocus');
      expect(attr).to.equal('autofocus');
    });

it('-4-(key, value) : should set attr', function () {
      var $pear = $('.pear').attr('id', 'pear');
      expect($('#pear')).to.have.length(1);
      expect($pear).to.be.a($);
    });

it('-5-(key, value) : should set attr', function () {
...
it('-6-(key, value) : should return an empty object for an empty object', function () {
      var $src = $().attr('key', 'value');
      expect($src.length).to.equal(0);
      expect($src[0]).to.be(undefined);
    });

it('-7-(map) : object map should set multiple attributes', function () {
      $('.apple').attr({
        id: 'apple',
        style: 'color:red;',
...
it('-8-(key, function) : should call the function and update the attribute with the return value', function () {
      var $fruits = $('#fruits');
...
it('-9-(key, value) : should correctly encode then decode unsafe values', function () {
      var $apple = $('.apple');
      $apple.attr(
        'href',
...
it('-10-(key, value) : should coerce values to a string', function () {
      var $apple = $('.apple');
...
it('-11-(key, value) : handle removed boolean attributes', function () {
      var $apple = $('.apple');
...
it('-12-(key, value) : should remove non-boolean attributes with names or values similar to boolean ones', function () {
      var $apple = $('.apple');
...
it('-13-(key, value) : should remove attributes when called with null value', function () {
...
it('-14-(map) : should remove attributes with null values', function () {
      var $pear = $('.pear').attr({
        autofocus: 'autofocus',
        style: 'color:red',
      });
...
it('-15-(valid key) : valid prop should get value', function () {
...
it('-16-(invalid key) : invalid prop should get undefined', function () {
      var attr = checkbox.prop('lol');
      expect(attr).to.be(undefined);
    });

it('-17-(key, value) : should set prop', function () {
      expect(checkbox.prop('checked')).to.equal(true);
...
it('-18-(map) : object map should set multiple props', function () {
      checkbox.prop({
        id: 'check',
        checked: false,
      });
...
it('-19-(key, function) : should call the function and update the prop with the return value', function () {
...
it('-20-(key, value) : should support chaining after setting props', function () {
      expect(checkbox.prop('checked', false)).to.equal(checkbox);
    });

it('-21-(invalid element/tag) : prop should return undefined', function () {
      expect($(undefined).prop('prop')).to.be(undefined);
      expect($(null).prop('prop')).to.be(undefined);
    });

it('-22-("outerHTML") : should render properly', function () {
...
it('-23-(inherited properties) : prop should support inherited properties', function () {
...
it('-24-() : should get all data attributes initially declared in the markup', function () {
      var data = $('.linth').data();
      expect(data).to.eql({
        highlight: 'Lindor',
        origin: 'swiss',
...
it('-25-() : should get all data set via `data`', function () {
      var $el = cheerio('<div>');
      $el.data('a', 1);
      $el.data('b', 2);
...
it('-26-() : should get all data attributes initially declared in the markup merged with all data additionally set via `data`', function () {
...
it('-27-() : no data attribute should return an empty object', function () {
      var data = $('.cailler').data();
      expect(data).to.be.empty();
    });

it('-28-(invalid key) : invalid data attribute should return `undefined` ', function () {
      var data = $('.frey').data('lol');
      expect(data).to.be(undefined);
    });

it('-29-(valid key) : valid data attribute should get value', function () {
...
it('-30-(key) : should translate camel-cased key values to hyphen-separated versions', function () {
      var $el = cheerio(
...
it('-31-(key) : should retrieve object values', function () {
      var data = {};
      var $el = cheerio('<div>');
...
it('-32-(key) : should parse JSON data derived from the markup', function () {
      var $el = cheerio('<div data-json="[1, 2, 3]">');

      expect($el.data('json')).to.eql([1, 2, 3]);
    });

it('-33-(key) : should not parse JSON data set via the `data` API', function () {
      var $el = cheerio('<div>');
      $el.data('json', '[1, 2, 3]');
...
it('-34-(key) : should ignore the markup value after the first access', function () {
...
it('-35-(key) : should recover from malformed JSON', function () {
      var $el = cheerio('<div data-custom="{{templatevar}}">');

      expect($el.data('custom')).to.be('{{templatevar}}');
    });

it('-36-(hyphen key) : data addribute with hyphen should be camelized ;-)', function () {
      var data = $('.frey').data();
      expect(data).to.eql({
        taste: 'sweet',
...
it('-37-(key, value) : should set data attribute', function () {
      // Adding as object.
      var a = $('.frey').data({
        balls: 'giandor',
      });
...
it('-38-(key, value) : should set data for all elements in the selection', function () {
      $('li').data('foo', 'bar');
...
it('-39-(map) : object map should set multiple data attributes', function () {
      var data = $('.linth').data({
        id: 'Cailler',
...
it('-46-(): on select should get value', function () {
      var val = $('select#one').val();
      expect(val).to.equal('option_selected');
    });
it('-47-(): on select with no value should get text', function () {
      var val = $('select#one-valueless').val();
      expect(val).to.equal('Option selected');
    });
it('-48-(): on select with no value should get converted HTML', function () {
      var val = $('select#one-html-entity').val();
      expect(val).to.equal('Option <selected>');
    });
it('-49-(): on select with no value should get text content', function () {
      var val = $('select#one-nested').val();
      expect(val).to.equal('Option selected');
    });
it('-50-(): on option should get value', function () {
      var val = $('select#one option').eq(0).val();
      expect(val).to.equal('option_not_selected');
    });
it('-51-(): on text input should get value', function () {
      var val = $('input[type="text"]').val();
      expect(val).to.equal('input_text');
    });
it('-52-(): on checked checkbox should get value', function () {
      var val = $('input[name="checkbox_on"]').val();
      expect(val).to.equal('on');
    });
it('-53-(): on unchecked checkbox should get value', function () {
      var val = $('input[name="checkbox_off"]').val();
      expect(val).to.equal('off');
    });
it('-54-(): on valueless checkbox should get value', function () {
      var val = $('input[name="checkbox_valueless"]').val();
      expect(val).to.equal('on');
    });
it('-55-(): on radio should get value', function () {
      var val = $('input[type="radio"]').val();
      expect(val).to.equal('off');
    });
it('-56-(): on valueless radio should get value', function () {
      var val = $('input[name="radio_valueless"]').val();
      expect(val).to.equal('on');
    });
it('-57-(): on multiple select should get an array of values', function () {
      var val = $('select#multi').val();
      expect(val).to.eql(['2', '3']);
    });
it('-58-(): on multiple select with no value attribute should get an array of text content', function () {
      var val = $('select#multi-valueless').val();
      expect(val).to.eql(['2', '3']);
    });
it('-59-(): with no selector matches should return nothing', function () {
      var val = $('.nasty').val();
      expect(val).to.equal(undefined);
    });
it('-60-(invalid value): should only handle arrays when it has the attribute multiple', function () {
      var val = $('select#one').val([]);
      expect(val).not.to.equal(undefined);
    });
it('-61-(value): on input text should set value', function () {
      var element = $('input[type="text"]').val('test');
      expect(element.val()).to.equal('test');
    });
it('-62-(value): on select should set value', function () {
      var element = $('select#one').val('option_not_selected');
      expect(element.val()).to.equal('option_not_selected');
    });
it('-63-(value): on option should set value', function () {
      var element = $('select#one option').eq(0).val('option_changed');
      expect(element.val()).to.equal('option_changed');
    });
it('-64-(value): on radio should set value', function () {
      var element = $('input[name="radio"]').val('off');
      expect(element.val()).to.equal('off');
    });
it('-65-(value): on radio with special characters should set value', function () {
      var element = $('input[name="radio[brackets]"]').val('off');
      expect(element.val()).to.equal('off');
    });
it('-66-(values): on multiple select should set multiple values', function () {
...
it('-67-(key) : should remove a single attr', function () {
      var $fruits = $('#fruits');
...
it('-68-should return cheerio object', function () {
...
it('-69-(valid class) : should return true', function () {
...
it('-70-(invalid class) : should return false', function () {
...
it('-71-should check multiple classes', function () {
      // Add a class
      $('.apple').addClass('red');
...
it('-72-(empty string argument) : should return false', function () {
...
it('-73-(first class) : should add the class to the element', function () {
      var $fruits = $('#fruits');
...
it('-74-(single class) : should add the class to the element', function () {
      $('.apple').addClass('fruit');
      var cls = $('.apple').hasClass('fruit');
      expect(cls).to.be.ok();
    });

it('-75-(class): adds classes to many selected items', function () {
      $('li').addClass('fruit');
...
it('-76-(class class class) : should add multiple classes to the element', function () {
...
it('-77-(fn) : should add classes returned from the function', function () {
...
it('-78-() : should remove all the classes', function () {
      $('.pear').addClass('fruit');
      $('.pear').removeClass();
      expect($('.pear').attr('class')).to.be(undefined);
    });

it('-79-("") : should not modify class list', function () {
      var $fruits = $('#fruits');
      $fruits.children().removeClass('');
      expect($('.apple')).to.have.length(1);
    });

it('-80-(invalid class) : should not remove anything', function () {
      $('.pear').removeClass('fruit');
      expect($('.pear').hasClass('pear')).to.be.ok();
    });

it('-81-(no class attribute) : should not throw an exception', function () {
...
it('-82-(single class) : should remove a single class from the element', function () {
      $('.pear').addClass('fruit');
      expect($('.pear').hasClass('fruit')).to.be.ok();
      $('.pear').removeClass('fruit');
...
it('-83-(single class) : should remove a single class from multiple classes on the element', function () {
...
it('-84-(class class class) : should remove multiple classes from the element', function () {
      $('.apple').addClass('fruit red tasty');
      expect($('.apple').hasClass('apple')).to.be.ok();
      expect($('.apple').hasClass('fruit')).to.be.ok();
      expect($('.apple').hasClass('red')).to.be.ok();
...
it('-85-(class) : should remove all occurrences of a class name', function () {
      var $div = cheerio('<div class="x x y x z"></div>');
      expect($div.removeClass('x').hasClass('x')).to.be(false);
    });

it('-86-(fn) : should remove classes returned from the function', function () {
      var $fruits = $('#fruits').children();
      var args = [];
      var thisVals = [];
      var toAdd = ['apple red', '', undefined];
...
it('-87-(fn) : should no op elements without attributes', function () {
      var $inputs = $(inputs);
...
it('-88-(class class) : should toggle multiple classes from the element', function () {
      $('.apple').addClass('fruit');
      expect($('.apple').hasClass('apple')).to.be.ok();
      expect($('.apple').hasClass('fruit')).to.be.ok();
...
it('-89-(class class, true) : should add multiple classes to the element', function () {
      $('.apple').addClass('fruit');
      expect($('.apple').hasClass('apple')).to.be.ok();
      expect($('.apple').hasClass('fruit')).to.be.ok();
      expect($('.apple').hasClass('red')).to.not.be.ok();
...
it('-90-(class true) : should add only one instance of class', function () {
      $('.apple').toggleClass('tasty', true);
      $('.apple').toggleClass('tasty', true);
      expect($('.apple').attr('class').match(/tasty/g).length).to.equal(1);
    });

it('-91-(class class, false) : should remove multiple classes from the element', function () {
      $('.apple').addClass('fruit');
      expect($('.apple').hasClass('apple')).to.be.ok();
      expect($('.apple').hasClass('fruit')).to.be.ok();
      expect($('.apple').hasClass('red')).to.not.be.ok();
...
it('-92-(fn) : should toggle classes returned from the function', function () {
      $ = cheerio.load(food);
...
it('-93-(fn) : should work with no initial class attribute', function () {
...
it('-94-(invalid) : should be a no-op for invalid inputs', function () {
      var original = $('.apple');
...
it('-103-(false predicate) : should return false', function () {
      var result = $('li')
        .last()
        .is(function () {
