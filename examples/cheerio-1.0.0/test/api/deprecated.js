...
it('-117-(html) : should preserve content', function () {
...
it('-118-should be a function', function () {
        expect(typeof cheerio.merge).to.equal('function');
      });

it('-119-(arraylike, arraylike) : should return an array', function () {
        var ret = cheerio.merge(arr1, arr2);
        expect(typeof ret).to.equal('object');
        expect(ret instanceof Array).to.be.ok();
      });

it('-120-(arraylike, arraylike) : should modify the first array', function () {
        cheerio.merge(arr1, arr2);
        expect(arr1).to.have.length(6);
      });

it('-121-(arraylike, arraylike) : should not modify the second array', function () {
        cheerio.merge(arr1, arr2);
        expect(arr2).to.have.length(3);
      });

it('-122-(arraylike, arraylike) : should handle objects that arent arrays, but are arraylike', function () {
        arr1 = {};
        arr2 = {};
        arr1.length = 3;
        arr1[0] = 'a';
...
it('-123-(?, ?) : should gracefully reject invalid inputs', function () {
...
it('-124-(?, ?) : should no-op on invalid inputs', function () {
        var fakeArray1 = { length: 3 };
        fakeArray1[0] = 'a';
        fakeArray1[1] = 'b';
        fakeArray1[3] = 'd';
...
it('-125-(container, contained) : should correctly detect the provided element', function () {
        var $food = $('#food');
        var $fruits = $('#fruits');
        var $apple = $('.apple');
...
it('-126-(container, other) : should not detect elements that are not contained', function () {
        var $fruits = $('#fruits');
...
it('-127-returns an empty selection', function () {
...
it('-129-() : of empty cheerio object should return null', function () {
...
it('-130-(selector) : should return the outerHTML of the selected element', function () {
...
it('-132-(cheerio object) : should return the text contents of the specified elements', function () {
        var $ = cheerio.load('<a>This is <em>content</em>.</a>');
        expect($.text($('a'))).to.equal('This is content.');
      });

it('-133-(cheerio object) : should omit comment nodes', function () {
        var $ = cheerio.load(
...
it('-134-(cheerio object) : should include text contents of children recursively', function () {
        var $ = cheerio.load(
...
it('-135-() : should return the rendered text content of the root', function () {
        var $ = cheerio.load(
          '<a>This is <div>a child with <span>another child and <!-- a comment --> not a comment</span> followed by <em>one last child</em> and some final</div> text.</a>'
        );
...
it('-136-(cheerio object) : should omit script tags', function () {
        var $ = cheerio.load('<script>console.log("test")</script>');
        expect($.text()).to.equal('');
      });

it('-137-(cheerio object) : should omit style tags', function () {
        var $ = cheerio.load(
...
it('-138-(cheerio object) : should include text contents of children omiting style and script tags', function () {
        var $ = cheerio.load(
