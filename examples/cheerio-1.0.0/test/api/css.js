...
it('-104-(prop): should return a css property value', function () {
      var el = cheerio('<li style="hai: there">');
      expect(el.css('hai')).to.equal('there');
    });

it('-105-([prop1, prop2]): should return the specified property values as an object', function () {
...
it('-106-(prop, val): should set a css property', function () {
...
it('-107-(prop, ""): should unset a css property', function () {
      var el = cheerio('<li style="padding: 1px; margin: 0;">');
      el.css('padding', '');
      expect(el.attr('style')).to.equal('margin: 0;');
    });

it('-108-(prop): should not mangle embedded urls', function () {
      var el = cheerio(
...
it('-114-should iterate over the selection', function () {
        var count = 0;
        var $el = this.$el;
...
it('-115-should set each attribute independently', function () {
...
it('-113-(obj): should set each key and val', function () {
...
it('-116-should allow any whitespace between declarations', function () {
